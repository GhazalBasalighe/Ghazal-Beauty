    ...
    images.map(async (image, index) => {
      ...
      await sharp(image.buffer)
        ...
        .toFile(
          join(
            __dirname,
            `../public/images/products/images/${imageFilename}`
          )
        );
...
const addProduct = async (req, res, next) => {
  ...
  if (!subcategory) {
    return next(
      new AppError(404, `subcategory: ${subcategory} not found`)
    );
  }
...
const editProductById = async (req, res, next) => {
  ...
  if (!!subcategoryId && !subcategory) {
    return next(
      new AppError(404, `subcategory: ${subcategoryId} not found`)
    );
  }
  ...
  const thumbnail = await resizeProductThumbnail(
    productId,
    req.files ?? {}
  );
